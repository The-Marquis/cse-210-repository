...
namespace greed_revision.Game.Casting
{
    /// <summary>
    ...
    public class Actor
    {
        ...
        public void MoveNext(int maxX, int maxY, string actor)
        {   
            int y = (position.GetY() + maxY) % maxY;
            int x = (position.GetX() + maxX) % maxX;
            if (actor == "player"){
                x = ((position.GetX() + velocity.GetX()) + maxX) % maxX;
                y = ((position.GetY()) + maxY) % maxY;
            }
            else if (actor == "artifacts"){
                x = ((position.GetX()) + maxX) % maxX;
                y = ((position.GetY() + 15) + maxY) % maxY;
            }
